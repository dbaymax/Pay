...
## 声明查询结果
	...
	$r6_Order				= "";			#	商户订单号
	$r8_MP					= "";			#	商户扩展信息
	$rw_RefundRequestID		= "";#	退款请求号
	$rx_CreateTime					= "";			#	订单创建时间
	$ry_FinshTime					= "";			#	订单成功时间
	$rz_RefundAmount					= "";			#	退款请求金额
	$rb_PayStatus		= "";			#	支付状态
	...
	for($index=0;$index<count($result);$index++){//数组循环
		...
		if($sKey=="r0_Cmd"){											#业务类型 
			...
			$r8_MP = $sValue;
		}elseif($sKey == "rw_RefundRequestID"){					#退款请求号
			$rw_RefundRequestID = $sValue;
		}elseif($sKey == "rx_CreateTime"){					#订单创建时间
			$rx_CreateTime = $sValue;
		}elseif($sKey == "ry_FinshTime"){					#订单成功时间
			$ry_FinshTime = $sValue;
		}elseif($sKey == "rz_RefundAmount"){					#退款请求金额
	...
	if($sNewString==$hmac) {
	  if($r1_Code=="1"){
	      ...
	      echo "<br>商户扩展信息:".$r8_MP;
		   echo "<br>退款请求号:".$rw_RefundRequestID;
		  echo "<br>订单创建时间:".$rx_CreateTime;
		  echo "<br>订单成功时间:".$ry_FinshTime;
		   echo "<br>退款请求金额:".$rz_RefundAmount;
